...
<html lang="en">
    ...
    <body class="truebody">
        <main class="mainbody centeredsection">
            ...
            <?php
            if (!file_exists($manifest_config["files"]["ignore"])) { // Check to see if the file doesn't already exists.
                file_put_contents($manifest_config["files"]["ignore"], "{}"); // Create a blank list file.
            }
            if (!is_writable($manifest_config["files"]["ignore"])) { // Check to see if the file isn't writable to PHP.
                echo "<p>Error: The file is not writable to PHP. Ignore lists can't be edited.</p>";
                exit();
            }

            $ignore_list = json_decode(file_get_contents($manifest_config["files"]["ignore"]), true); // Load the ignore list contents from the file.


            if ($_POST["submit"] == "Add") {
                array_push($ignore_list, strtoupper($_POST["plate"]));
                file_put_contents($manifest_config["files"]["ignore"], json_encode($ignore_list, JSON_PRETTY_PRINT)); // Save the modified list to disk.
            } else if ($_POST["submit"] == "Remove") { // Check to see if a plate to remove was submitted.
                if (($key = array_search($_POST["plate"], $ignore_list)) !== false) {
