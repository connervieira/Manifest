...
<html lang="en">
    ...
    <body class="truebody">
        <main class="mainbody centeredsection">
            ...
            <?php
            if (!file_exists($manifest_config["files"]["hotlist"])) { // Check to see if the file doesn't already exists.
                file_put_contents($manifest_config["files"]["hotlist"], "{}"); // Create a blank list file.
            }
            if (!is_writable($manifest_config["files"]["hotlist"])) { // Check to see if the file isn't writable to PHP.
                echo "<p>Error: The file is not writable to PHP. Hot lists can't be edited.</p>";
                exit();
            }

            $hotlist = json_decode(file_get_contents($manifest_config["files"]["hotlist"]), true); // Load the hot list contents from the file.


            if ($_POST["submit"] == "Add") {
                $plate_to_add = strtoupper($_POST["plate"]); // Get the plate to add to the hot list from the POST data.
                $hotlist[$plate_to_add]["name"] = $_POST["name"];
                $hotlist[$plate_to_add]["description"] = $_POST["description"];
                $hotlist[$plate_to_add]["author"] = $_POST["author"];
                $hotlist[$plate_to_add]["source"] = $_POST["source"];
            ...
            <div class="basicform">
                ...
                <form method="POST">
                    ...
                    <label for="name">Name:</label> <input type="string" name="name" id="name" placeholder="Name" value="<?php echo $hotlist[$_GET["plate"]]["name"]; ?>"><br>
                    <label for="description">Description:</label> <input type="string" name="description" id="description" placeholder="Description" value="<?php echo $hotlist[$_GET["plate"]]["description"]; ?>"><br>
                    <label for="author">Author:</label> <input type="string" name="author" id="author" placeholder="Author" value="V0LT" value="<?php echo $hotlist[$_GET["plate"]]["author"]; ?>"><br>
                    <label for="source">Source:</label> <input type="string" name="source" id="source" placeholder="Source" value="<?php echo $hotlist[$_GET["plate"]]["source"]; ?>"><br>
